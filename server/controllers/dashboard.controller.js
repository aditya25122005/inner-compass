...
const getDashboardData = asyncHandler(async (req, res) => {
    ...
    const journalEntries = await Journal.find({ user: userId }).select('analysis mood createdAt').sort({ createdAt: -1 });
    const tasks = await Task.find({ user: userId }).select('isCompleted assignedDate').sort({ assignedDate: -1 });
    const metrics = calculateMetrics(journalEntries, tasks, req.user);
    res.status(200).json({
        success: true,
        data: metrics
    });
